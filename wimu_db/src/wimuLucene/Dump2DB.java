...
public class Dump2DB {
	...
	public static long lim = 700000000;
  //public static long lim = 100000;
	...
	public static void execMain(String args[]) throws IOException, ClassNotFoundException, SQLException {
		System.out.println("Dumb2DB...Parallel");
	...
	private static void insertDB(ConcurrentHashMap<String, Integer> mDatatypeTriples) {
		System.out.println("Insert DB...mDataTypeTriples.size:" + mDatatypeTriples.size());
		long startTime = System.currentTimeMillis();
		mDatatypeTriples.forEach((uriDataset, dTypes) -> {
			try {
				String s[] = uriDataset.split("\t");
				String uri = s[0];
				String dataset = s[1];
				insertDB(uri, dTypes, dataset);
			} catch (Exception e) {
	...
	private static void insertDB(String uri, Integer dTypes, String dataset) throws SQLException, ClassNotFoundException {
				...
				public void triple(Triple triple) {
					...
					if (triple.getObject().isLiteral()) {
						...
						if(mDatatypeTriples.size() > lim){
							...
							String fileName = fUnzip.getDataset().replaceAll("/", ";");
							insertDB(mDatatypeTriples);
