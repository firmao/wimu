...
public class Dump2Solr {
	...
	public static void create(String pDumpDir) throws IOException, SolrServerException {
...
//		solr.commit();
		...
		for (String fileURL : setAllFileURLs) {
			...
			setFiles.parallelStream().forEach(file -> {
				...
				if (processCompressedFile(file, provenance, solr)) {
		...
		date = new Date();
		System.out.println("Dump2Solr...Parallel, finalize at: " + dateFormat.format(date));
	}

	private static void exitBefore(HttpSolrClient solr) throws IOException, SolrServerException {
	...
	private static boolean processCompressedFile(FileWIMU file, String dataset, HttpSolrClient solr) {
		boolean ret = false;
		try {
			...
			if (file.getName().endsWith(".bz2"))
				...
				return processUnzipRDF(file, solr);
			...
			if (fUnzip != null)
				ret = processUnzipRDF(fUnzip, solr);
		} catch (Exception ex) {
			ret = false;
	...
	private static boolean processUnzipRDF(FileWIMU fUnzip, HttpSolrClient solr) {
		boolean ret = true;
		try {
				...
				public void triple(Triple triple) {
					...
					if (triple.getObject().isLiteral()) {
						...
						if (mDatatypeTriples.size() > lim) {
							...
							System.out.println("Reach the limit: " + lim + " Inserting to Solr");
							try {
								SolrUtil.createIndex(mDatatypeTriples, solr);
